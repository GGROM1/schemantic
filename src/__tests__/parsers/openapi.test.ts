...
describe('OpenAPIParser', () => {
  const createConfig = (): TypeSyncConfig => createTestConfig({
  ...
  describe('parse', () => {
    ...
    it('should throw error for invalid input', async () => {
      await expect(parser.parse({})).rejects.toThrow('No valid input source provided');
    });
  ...
  describe('validate', () => {
    ...
    it('should detect unsupported OpenAPI version', async () => {
      ...
      const result = await parser.validate(invalidSchema as any);
    ...
    it('should detect invalid paths', async () => {
      ...
      expect(result.isValid).toBe(false);
      expect(result.errors.some(error => error.code === 'INVALID_PATH_ITEM')).toBe(true);
    });
    ...
    it('should detect missing operation responses', async () => {
      ...
      expect(result.isValid).toBe(false);
      expect(result.errors.some(error => error.code === 'MISSING_RESPONSES')).toBe(true);
    });
    ...
    it('should detect invalid schemas', async () => {
      ...
      expect(result.isValid).toBe(false);
      expect(result.errors.some(error => error.code === 'INVALID_SCHEMA')).toBe(true);
    });
  });
