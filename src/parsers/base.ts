...
import { TypeSyncConfig, GenerationContext } from '../types/core';
 ...
 */
export abstract class BaseSchemaParser<T = OpenAPISchema> implements SchemaParser<T> {
   ...
   */
  protected resolveReference(ref: string, schema: T): ResolvedSchema | undefined {
    if (!ref.startsWith('#/')) {
      return undefined;
    }
   ...
   */
  protected traverseSchema(schema: T, path: string[]): ResolvedSchema | undefined {
  ...
  protected createGenerationContext(schema: T): GenerationContext {
    ...
    const generatedClients = new Map<string, any>();
   ...
   */
  private createTypeRegistry(): any {
