...
export class OpenAPIParser extends BaseSchemaParser<OpenAPISchema> {
  private readonly SUPPORTED_VERSIONS = ['3.0.0', '3.0.1', '3.0.2', '3.0.3', '3.1.0'];
  ...
  createResolver(schema: OpenAPISchema): (ref: string) => ResolvedSchema | undefined {
  ...
  getMetadata(): ParserMetadata {
    return {
      ...
      description: 'Parser for OpenAPI 3.0+ specifications from FastAPI applications',
    };
  }
   ...
   */
  private validatePaths(paths: OpenAPIPaths, errors: ValidationError[], warnings: ValidationWarning[]): void {
    for (const [pathKey, pathItem] of Object.entries(paths)) {
      ...
      if (pathItem.parameters) {
        this.validateParameters(pathItem.parameters, `paths.${pathKey}.parameters`, errors, warnings);
      }
  ...
  private validateOperation(
    ...
    if (operation.parameters) {
      this.validateParameters(operation.parameters, `${path}.parameters`, errors, _warnings);
    }
  ...
  private validateComponents(
    ...
    if (components.schemas) {
      this.validateSchemas(components.schemas, 'components.schemas', errors, _warnings);
    }
    ...
    for (const componentType of componentTypes) {
      if (components[componentType]) {
        this.validateComponentType(components[componentType], `components.${componentType}`, errors, _warnings);
      }
    }
  }
  ...
  private validateSchemas(
    ...
    for (const [schemaName, schema] of Object.entries(schemas)) {
      ...
      if (this.hasCircularReference(schema, new Set())) {
        _warnings.push({
   ...
   */
  private hasCircularReference(obj: any, visited: Set<string>): boolean {
